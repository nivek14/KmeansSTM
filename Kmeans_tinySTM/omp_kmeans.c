        ...
        if (is_perform_atomic) {
            ...
            #pragma omp parallel for \
				...
				for (i=0; i<numObjs; i++) {
					...
					stm_store_int(&newClusterSize[index], stm_load_int(&newClusterSize[index]) + 1);
					STM_TRY_COMMIT();
					//newClusterSize[index]++;
					for (j=0; j<numCoords; j++) {
					   // #pragma omp atomic
						STM_START_TRANSACTION();
						stm_store_float(&newClusters[index][j], stm_load_float(&newClusters[index][j]) + objects[i][j]);
